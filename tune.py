...
def objective(trial, features, raw_df, device, model_type="LSTM_AE"):
    # Suggest common hyperparams
    window_size = trial.suggest_categorical("window_size", [60, 90, 120])
    batch_size = trial.suggest_categorical("batch_size", [32, 64, 128])
    lr = trial.suggest_float("lr", 1e-4, 1e-2, log=True)   # ✅ fixed

    if model_type == "LSTM_AE":
        hidden_size = trial.suggest_categorical("hidden_size", [16, 32, 64, 128])
        dropout = trial.suggest_float("dropout", 0.1, 0.5)   # ✅ fixed
        ...
        dropout = trial.suggest_float("dropout", 0.1, 0.4)   # ✅ fixed
    ...
    # Train (✅ unpack all returns safely)
    ...
    # Evaluate anomalies (optional, not used in Optuna objective but logged)
